...
const returnPromise = (req: any) => new Promise((resolve) => {
  req.on('response', () => {
...
const getParams = (
  ...
  for (const [fieldName, fieldValues] of arrayToCheck) {
    params.push(
      `${
        (params.length || isQueryParams ? '&' : '?') + objOfFields[fieldName]
      }=${fieldValues.join(',')}`,
    )
  }
...
const getArrayFields = (fields?: Fields | FieldsFollowers) => (fields ? Object.entries(fields) : [])
...
export {
  returnPromise, getParams, checkFields, getArrayFields, getApi,
}
